function [] = plot_all_2nd(bitstream,T,snr_dB_range,beta)
    ...
    for i = 1: length(snr)
        for j = 1: 30
        [recv_sign,~,~,~,~,a] = bsc_channel(bitstream,3,T,b(1),rho(1),1,snr(i),beta);
        [~,~,BER_temp] = judging(3,recv_sign,3,a,T,bitstream,0);
        BER(i) = BER(i) + BER_temp;
        [~,~,BER_2_t] = judging(2,recv_sign,3,a,T,bitstream,0);
        
        [recv_sign_2,~,~,~,~,a_2] = bsc_channel(bitstream,3,T,b(2),rho(2),1,snr(i),beta);
        [~,~,BER_temp_3] = judging(3,recv_sign_2,3,a_2,T,bitstream,0);
        
%         [recv_sign_3,~,~,~,~,~] = bsc_channel(bitstream,2,T,b,rho,1,snr(i),beta);
        [~,~,BER_temp_4] = judging(2,recv_sign_2,3,a_2,T,bitstream,0);
        
        [recv_sign_4,~,~,~,~,a_3] = bsc_channel(bitstream,3,T,b(3),rho(3),1,snr(i),beta);
...
%    legend('bit_num = 3,b = 1,rho = 1, scene 2');
    legend('bit_num = 3,b = 0,rho = 0, scene 3','bit_num = 3,b = 0,rho = 0, scene 2',...
        'bit_num = 3,b = 0.7,rho = 0.996, scene 3','bit_num = 3,b = 0.7,rho = 0.996, scene 2',...
        'bit_num = 3,b = 1,rho = 1, scene 3','bit_num = 3,b = 1,rho = 1, scene 2');
    xxxxx=0;
end